...
//  instrument at program exit point to save code coverage samples as files
...
static const char *USAGE = 
  " [-bfpsa] <binary> <output binary>\n \
   -b: Basic block level code coverage\n \
   -f: Save code coverage samples as file\n \
...
static const char *OPT_STR = "bfpsa";
...
bool insertFuncEntry( BPatch_binaryEdit * appBin, BPatch_function *curFunc, char *funcName, BPatch_function *instIncFunc, int funcId)
{
  ...
  BPatchSnippetHandle *handle = appBin->insertSnippet (instIncExpr, *funcEntry, BPatch_callBefore, BPatch_lastSnippet);
  if (!handle) {
...
bool insertBBEntry( BPatch_binaryEdit * appBin, BPatch_function *curFunc, char *funcName, const char *moduleName, BPatch_function * instBBIncFunc,
        BPatch_function *registerBB, int *bbIndex, BPatch_Vector <BPatch_snippet *> *registerCalls)
{
  ...
  for (iter = allBlocks.begin(); iter != allBlocks.end(); iter++) {
    ...
    BPatchSnippetHandle *handle = appBin->insertSnippet (instIncExpr, *bbEntry, BPatch_callBefore, BPatch_lastSnippet);
    if (!handle) 
      {
...
int main (int argc, char *argv[])
{
  ...
  BPatch_function * instInitFunc = findFuncByName(appImage, (char *)"initCoverage");
  BPatch_function * registerFunc = findFuncByName(appImage, (char *)"registerFunc");
  BPatch_function * instIncFunc = findFuncByName(appImage, (char *)"incFuncCoverage");
  BPatch_function * registerBB = findFuncByName(appImage, (char *)"registerBB");
  BPatch_function * instBBIncFunc = findFuncByName(appImage, (char *)"incBBCoverage");
  BPatch_function * instExitFunc = findFuncByName(appImage, (char *)"exitCoverage");
  ...
  BPatch_constExpr varInBinary(saveSamples ? inBinary : "");
  instExitArgs.push_back(&varInBinary);
